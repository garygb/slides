 ...
 */
int bitCount(int x) {
  int mask = ((0x1<<8|0x1)<<8|0x1)<<8|0x1;
  int cnt = mask & x;
  int nBits = 0;
  cnt += mask & (x >> 1);
  cnt += mask & (x >> 2);
 ...
 */
int bang(int x) {
  //if x != 0, the highest bit of x and (!x + 1) can't be both 0
  //it means highestBit( x|(!x + 1) ) == 1
  return (~(x|(~x+1)) >> 31) & 0x1;
}
/* 
...
int fitsBits(int x, int n) {
  int isPositive = !(x >> 31); //this "positive" stands for an int >= 0
...
int divpwr2(int x, int n) {
    ...
    int signx = x >> 31;
    int mask = (1 << n) + (~0);  //(equal to "11..1" with length = n)
    int bias = signx & mask;
    return (x + bias) >> n;
}
/* 
...
int isLessOrEqual(int x, int y) {
  int signx = x >> 31;
  int signy = y >> 31;
  int signSame = ((x + ~y) >> 31) & !(signx^signy);  // x-y <= 0 and x,y have same sign 
  int signDiffer = signx & !signy;   // x < 0 and y >= 0
  return (signSame | signDiffer);
}
/*
 ...
 *   Rating: 4
 */
int ilog2(int x) { //find the MSB 1(return the distance between it and the end)
	int bitsNumber=0;  
 ...
 *   Rating: 4
 */
unsigned float_i2f(int x) { //check
